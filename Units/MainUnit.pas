...
var
  ...
  DPort, WPort:Byte;
  Decimal:char;
const demover:Boolean = false; //Демо верися.
  KeyBtn: array[0..61] of Char = ('а','б','в','г','д','е','ж','з','и','й','к','л','м','н','о','п',
...
{ if (dm.Current_orderISBALANS.AsInteger = 1) and (dm.Current_orderMACCOUNT.AsCurrency < cxGrid1DBTableView1.DataController.Summary.FooterSummaryValues[0]) then
  begin
    MessageDlg('Лимит превышен!'+#13+'Остаток: '+FormatCurr('0.0', dm.Current_orderMACCOUNT.AsCurrency), mtError, [mbOk], 0);
    exit;
  end;  }

 if (dm.Orders_item.Active = true) and (dm.Orders_item.RecordCount = 0) then
  begin
   if MessageDlg('Удалить счет?',mtConfirmation,[mbYes,mbNo],0) = mrYes then
     begin
...
begin
  ...
  if Hndl = 0 then
    begin
      ShowMessage('ќшибка загрузки dll "'+TButton(Sender).Hint+'"');
    end
    else
    begin
...
begin
if dm.Orders_item.RecordCount=0 then exit;
if ACellViewInfo.Item.Caption = 'Кол-во' then
 begin
...
AviewInfo.Text:=Format('Остаток: %n', [saldo]);
if saldo < 0 then
  ...
  if ASender.FooterSummaryValues[0] = null then
   begin
    ...
    begin
     ...
     Label3.Caption:='Наценка: '+Format('%n',[StrToFloat(ASender.FooterSummaryValues[2])]);
     Label4.Caption:='Скидка: '+Format('%n',[StrToFloat(ASender.FooterSummaryValues[1])]);
    end;
end;
...
begin
 if Demo_t < GetTickCount then
  begin
    ...
    MessageDlg('Вы используете Демо-версию прогрммы.'+#13'Информация по приобретению лицензии на сайте разработчика www.aksau.kz',mtInformation,[mbOk],0);
...
if Demover then
 ...
 begin
  @f_CheckLic := nil;
    ...
    begin
      if not FileExists(ExtractFilePath(ParamStr(0)) + 'sys.lic') then
        try
          ...
          begin
            if not FileExists(ExtractFilePath(ParamStr(0)) + 'sys.lic') then
              MessageDlg('Доступ не активирован!', mtWarning, [mbOk], 0);
            Application.Terminate;
            FreeLibrary(Hlis);
            exit;
          end;
          ...
          MessageBox(Application.Handle,
            'Версия библиотеки License.dll должна быть больше 3.0.0.0',
        ...
        try
          ...
          MessageBox(Application.Handle,
            'Версия библиотеки License.dll должна быть больше 4.0.0.0',
            'Ошибка!', mb_IconWarning or mb_OK);
          Application.Terminate;
        end;
    end
    ...
    begin
      MessageDlg('Проргамма некомплектна!' + #13 + 'Переустановите программу!',
        mtWarning, [mbOk], 0);
      Application.Terminate;
    end;
    FreeLibrary(Hlis);
  ...
  try
   ...
   if DBFile = '' then
         begin
          MessageDlg('Внимание! Отстуствует информация о базе данных!'+#13+'Для продолжения работы укажите файл базы данных.',mtWarning,[mbOk],0);
         ...
         begin
           ...
           if dm.FDB.Connected then
              begin
               ...
               if Station = 0 then
                  begin
                    MessageDlg('Нет данных о станции!',mtWarning, [mbOk],0);
                    Application.Terminate;
                    exit;
                  end;
              end else
...
{ if dm.Fdb.ExecSQLScalar('select max(DT) from DOCUMENTS where OPERACII_TYPE = 2')>now then
  begin
    if MessageDlg('Внимание системное время не корректно!'+#13+'Все равно продолжить?',mtError, [mbYes,mbNo], 0) <> mrYes then
      Application.Terminate;
  end; }
...
//**************************************************
  ...
  prnts.DelimitedText:=tbStationPrinter.AsString;
   ...
   if tbStationBCARD_KLIENT.AsBoolean then   //Доступность Выбирать ли клиента по карте
   begin
...
if tbStationBCHANG_SCREEN.AsBoolean then      //Изменяем разререшение экрана на 800х600
 begin
...
begin
  ...
  if Handle = 0 then
    begin
      ShowMessage('Oшибка загрузки dll "'+AFileName+'"');
    end
    else
    try
...
begin
  if Length(S) >= Len
    then Result := Copy(S, 1, Len) // именно так работает ораклевый LPAD
    else Result :=
