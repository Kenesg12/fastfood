...
uses
  ...
  FireDAC.Comp.Client,
  cxButtons,
  ShowReport in 'ShowReport.pas' {fmShowReport},
...
begin
 ...
 for i := 0 to Application.ComponentCount - 1 do
  ...
  if DUID = '' then
   exit;
  if D.ExecSQLScalar('select count(*) from D_LINES where IDDOCUMENT = '''+DUID+''' and bdel = 0') = 0 then
   exit;
  Screen := S;
  fdb:=D;
  fmShowReport:=TfmShowReport.Create(Application.MainForm);
  Result:=fmShowReport.Handle;

  if fmShowReport.ShowModal = 1 then
   begin
     paybtn.Tag:=1;
     paybtn.Click;
